...
GLUquadric* qobj;
 GLuint selectBuf[SIZE];
   ...
   GLint viewport[4];
int obstacles[200][2];
int flag=0,flag_pos=0,flag_exit=0,game =0;
float bx=75,by=300;
...
void myReshape(int w, int h)
{
    ...
    glLoadIdentity();
    if (w <= h)
	glOrtho (0+moveFactor, 500.0+moveFactor, -0.0*(GLfloat)h/(GLfloat)w,
	    500.0*(GLfloat)h/(GLfloat)w, -10.0, 10.0);
    else
	glOrtho (-0.0*(GLfloat)w/(GLfloat)h+moveFactor,
	    500.0*(GLfloat)w/(GLfloat)h+moveFactor, -0.0, 500.0, -10.0, 10.0);
